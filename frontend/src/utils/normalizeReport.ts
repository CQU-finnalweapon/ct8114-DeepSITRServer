...
function normalizeLevel(value: any): string {
  ...
  if (text === "1" || /^error$/i.test(text) || /严重|错误/.test(text)) return "Error";
...
function normalizeDiagnostic(item: any, index: number): NormalizedDiagnostic {
  const track = Array.isArray(item?.tracking_path_list) ? item.tracking_path_list[0] : undefined;
  const location = item?.location || {};
  const trackLocation = track?.location_start || track?.location || {};
  const ruleId = String(firstValue(
    item?.rule_id,
    item?.ruleId,
    item?.check,
    item?.checker,
  ...
  const checker = String(firstValue(item?.checker, item?.check, item?.name, item?.type, ruleId, ""));
  ...
  const filePath = String(firstValue(
    item?.file,
    item?.file_path,
    item?.filePath,
    item?.path,
  ...
  const line = toNumber(firstValue(item?.line, location?.line, trackLocation?.line));
  const column = toNumber(firstValue(item?.column, location?.column, trackLocation?.column));
  const level = normalizeLevel(firstValue(item?.level, item?.severity, item?.force, item?.type, "Warning"));
...
function collectDiagnostics(raw: any): NormalizedDiagnostic[] {
  ...
  return list.map((item: any, index: number) => normalizeDiagnostic(item, index));
}
...
function inferFileCount(raw: any, diagnostics: NormalizedDiagnostic[]) {
  ...
  return toNumber(firstValue(
    ...
    report?.total_files,
    report?.file_count,
    Array.isArray(report?.files_stats) ? report.files_stats.length : undefined,
...
function makeMessage(raw: any, status: string, count: number) {
  ...
  if (status === "check_progress_empty") return "check_progress 暂无缺陷数据，可展开原始 JSON 排查";
...
export function normalizeReport(raw: any): NormalizedReport {
  ...
  const warning = diagnostics.filter((item) => /^warn/i.test(item.level)).length;
  ...
  const status = String(firstValue(raw?.status, raw?.report?.status, raw?.state, raw?.report?.state, diagnostics.length ? "completed" : "unknown"));
  const detectionId = firstValue(raw?.detection_id, raw?.detectionId, raw?.report?.detection_id, raw?.report?.detectionId);
  const requestId = String(firstValue(raw?.request_id, raw?.requestId, raw?.report?.request_id, raw?.report?.report_id, detectionId, ""));
