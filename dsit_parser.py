...
def _normalize_standard_severity(value: Any) -> str:
    text = str(value or "").strip().lower()
    if not text:
        return ""
    if text in {"required", "must_fix", "mandatory", "error", "1"} or "必须" in text or "强制" in text:
        return "required"
    if text in {"advisory", "warning", "warn", "0"} or "普通" in text or "警告" in text or "推荐" in text:
        return "advisory"
    if text in {"document", "documentation"}:
        return "unknown"
    if text in {"other", "note", "info"}:
        return "other"
    if text == "unknown":
        return "unknown"
    return "unknown"
...
def normalize_severity(
    *,
    ...
    type_code: Any = None,
    status: Any = None,
    source_prefix: str = "dcab",
) -> Dict[str, Optional[str]]:
    raw_text = str(raw_severity).strip() if raw_severity not in (None, "") else None
    if raw_text:
        severity = _normalize_standard_severity(raw_text)
        source = (
            f"{source_prefix}.required_advisory"
            if raw_text.lower() in {"required", "advisory"}
            else f"{source_prefix}.severity"
        )
        return {
            "severity": severity,
            "severity_label": SEVERITY_LABELS[severity],
            "severity_source": source,
            "raw_severity": raw_text,
        }

    force_text = str(force).strip() if force not in (None, "") else ""
    if force_text == "1":
        return {
            "severity": "required",
            "severity_label": SEVERITY_LABELS["required"],
            "severity_source": f"{source_prefix}.force",
            "raw_severity": None,
        }
    if force_text == "0":
        return {
            "severity": "advisory",
            "severity_label": SEVERITY_LABELS["advisory"],
            "severity_source": f"{source_prefix}.force",
            "raw_severity": None,
        }

    if str(type_code or "").strip() or str(status or "").strip():
        return {
            "severity": "other",
            "severity_label": SEVERITY_LABELS["other"],
            "severity_source": f"{source_prefix}.type/status",
            "raw_severity": None,
        }

    return {
        "severity": "unknown",
        "severity_label": SEVERITY_LABELS["unknown"],
...
@dataclass
class DSITBug:
    """Single diagnostic result aligned with frontend diagnostic cards."""
    checker: str
    file_path: str
    line: int
    column: int
    ...
    def __post_init__(self) -> None:
        ...
        self.severity = normalized["severity"] or "unknown"
        self.severity_label = normalized["severity_label"] or SEVERITY_LABELS["unknown"]
        self.severity_source = self.severity_source or normalized["severity_source"] or "fallback"
        self.raw_severity = normalized["raw_severity"]
        self.force = str(self.force or "")
        self.type_code = str(self.type_code or "")
        self.status = str(self.status or "")

    @property
    def level(self) -> str:
        """Compatibility display level for older clients."""
        if self.severity == "required" or self.force == "1":
            return "Error"
        if self.severity == "advisory" or self.force == "0":
            return "Warning"
        if self.severity == "other":
            return "Other"
        return "Unknown"

    def to_report_dict(self, file_path: Optional[str] = None, rule_id: Optional[str] = None) -> Dict[str, Any]:
        return {
            ...
            "rule_id": rule_id if rule_id is not None else _safe_rule_id(self.rule_id, self.checker),
            "level": self.level,
...
class DSITReport:
    ...
    def summary(self) -> Dict[str, Any]:
        """Generate aggregate summary for frontend display."""
        by_checker: Dict[str, int] = {}
        by_level: Dict[str, int] = {}
        by_severity: Dict[str, int] = {}
        by_severity_label: Dict[str, int] = {}
        ...
        for fs in self.files_stats:
            ...
            for bug in fs.bugs:
                ...
                by_severity_label[bug.severity_label] = by_severity_label.get(bug.severity_label, 0) + 1
                by_rule[rule_id] = by_rule.get(rule_id, 0) + 1
                all_bugs.append(bug.to_report_dict(file_path=bug_path, rule_id=rule_id))
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Serialize the full report as a dict."""
        return {
            ...
            "files_stats": [
                {
                    ...
                    "functions": [dict(func) for func in fs.functions],
    ...
    for base, xplusx_path in sorted(xplusx_files.items()):
        ...
        bugs = [
            DSITBug(
                ...
                severity_label=b.get("severity_label", ""),
                severity_source=b.get("severity_source", ""),
                raw_severity=b.get("raw_severity"),
            )
            for b in bugs_raw
        ]
...
def _parse_codetidy_output(
    ...
    for line in combined:
        ...
        bugs.append(DSITBug(
            ...
            status="0",
            severity_source="codetidy.level",
        ))
